...
| 2026-09-22 | `e29b18c` → `63771a5` | Reviews section: bold eyebrow, More/Less on long quotes, fade-in on scroll. CI green on both legs. No migration, no dependency change. **Asset rebuild** (`app-CumdkND0.css`), verified served, old file 404; the fade and toggle markup verified on the live page. Worker active. `/login` and `/` 200, 82 migrations. |
| 2026-09-22 | `63771a5` → `604ed5b` | Rich-text review quotes, pill Show more on the right, slower fade-in. CI green on both legs. No migration, no dependency change. **Asset rebuild** (`app-Doq4xwTm.css`), verified served, old file 404; the quote and toggle markup verified on the live page. Worker active. `/login` and `/` 200, 82 migrations. |
| 2026-09-22 | `604ed5b` → `03f05b7` | Homepage footer contacts split from Settings › Contact. CI green on both legs. One migration (copies the active Contact rows into the footer's own list, icons included; Contact rows untouched). No dependency change, no asset change. Footer contacts verified present on the live page after the copy. Worker active. `/login` and `/` 200, 83 migrations. |
| 2026-09-22 | `03f05b7` → `4c75b3a` | Contact status with Activate/Deactivate; fade over review quotes removed. CI green on both legs. No migration, no dependency change. **Asset rebuild** (`app-BBU4bJpB.css`), verified served, old file 404; no mask on the live page; the two contact routes verified behind `permission:contact.edit`. Worker active. `/login` and `/` 200, 83 migrations. |
| 2026-09-22 | `4c75b3a` → `62ac320` | Homepage header: "Student Login" button with icon, larger brand and button, taller bar. CI green on both legs. No migration, no dependency change. **Asset rebuild** (`app-BUyzAJXC.css`), verified served, old file 404; the button text verified on the live page. Worker active. `/login` and `/` 200, 83 migrations. |
| 2026-09-22 | `62ac320` → `13df583` | Plain login redirect after a sign-in elsewhere; icon hint dropped from the footer editor. CI green on both legs. No migration, no dependency change, no asset change. Worker active. `/login` and `/` 200, the notice verified gone, 83 migrations. |
| 2026-09-22 | `13df583` → `279a460` | Homepage floating contacts with a per-contact tick in the editor. CI green on both legs. No migration, no dependency change, no asset change. The floater verified on the live homepage. Worker active. `/login` and `/` 200, 83 migrations. |
| 2026-09-22 | `279a460` → `8577124` | Six-character password floor on the user form and the account page. CI green on both legs. PHP only: no migration, no dependency change, no asset change. Worker active. `/login` and `/` 200, 83 migrations. |
| 2026-09-22 | `8577124` → `8dbb0a1` | Public header fits a phone (medium brand, compact non-wrapping button on small screens). CI green on both legs. No migration, no dependency change. **Asset rebuild** (`app-BoWZ_ywF.css`), verified served, old file 404; the responsive markup verified on the live page. Worker active. `/login` and `/` 200, 83 migrations. |
| 2026-09-22 | `8dbb0a1` → `ba4f530` | Header button: "Login" on phones, "Student Login" from tablets up. CI green on both legs. Blade only: no migration, no dependency change, no asset change. Both labels verified on the live page. Worker active. `/login` and `/` 200, 83 migrations. |
